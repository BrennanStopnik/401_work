...
ADD FOREIGN KEY (book_PublisherName) REFERENCES tbl_publisher(publisher_PublisherName);
...
add foreign key (book_loans_BookID) references tbl_book(book_BookID);
alter table tbl_book_loans
add foreign key (book_loans_BranchID) REFERENCES tbl_library_branch(library_branch_BranchID);
alter table tbl_book_loans
add foreign key (book_loans_CardNo) REFERENCES tbl_borrower(borrower_CardNo);
